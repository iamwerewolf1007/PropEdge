...
try:
    from nba_api.stats.endpoints import ScoreboardV2, BoxScoreTraditionalV2, LeagueGameLog
...
def fetch_box_scores(game_date):
    """Fetch all box scores for games on a given date."""
    ...
    for gid in game_ids:
        try:
            ...
            home_abr = TEAM_ID_TO_ABR.get(home_id, '')
            away_abr = TEAM_ID_TO_ABR.get(away_id, '')

            for _, ps in player_stats.iterrows():
                if ps['MIN'] is None or ps['MIN'] == '' or ps['MIN'] == '0:00':
                    continue  # DNP

                team_id = ps['TEAM_ID']
                team_abr = TEAM_ID_TO_ABR.get(team_id, '')
                ...
                log = {
                    ...
                    'Points': int(ps['PTS'] or 0),
                    'FGM': int(ps['FGM'] or 0),
                    'FGA': int(ps['FGA'] or 0),
                    'FG%': round(float(ps['FG_PCT'] or 0) * 100, 1),
                    '3PM': int(ps['FG3M'] or 0),
                    '3PA': int(ps['FG3A'] or 0),
                    '3P%': round(float(ps['FG3_PCT'] or 0) * 100, 1),
                    'FTM': int(ps['FTM'] or 0),
                    'FTA': int(ps['FTA'] or 0),
                    'FT%': round(float(ps['FT_PCT'] or 0) * 100, 1),
                    'REB': int(ps['REB'] or 0),
                    'AST': int(ps['AST'] or 0),
                    'STL': int(ps['STL'] or 0),
                    'BLK': int(ps['BLK'] or 0),
                    'TOV': int(ps['TO'] or 0),
                    '+/-': int(ps['PLUS_MINUS'] or 0),
                    'W/L': 'W' if (is_home and game_row.get('HOME_TEAM_WINS',0)) or
                                  (not is_home and not game_row.get('HOME_TEAM_WINS',0)) else 'L',
                    'Game_ID': gid,
                }
                all_logs.append(log)

            print(f"    ✓ {home_abr} vs {away_abr}: {len(player_stats)} players")
